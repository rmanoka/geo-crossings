use geo::{Coordinate, GeoFloat, GeoNum, LineString, winding_order::{WindingOrder, Winding}};
use itertools::Either;
use log::{trace, debug};
use std::collections::{BTreeMap, VecDeque};

use crate::{line_or_point::LineOrPoint, SweepPoint, utils::winding_inverse};
...
impl<T: GeoFloat> Rings<T> {
    ...
    pub fn add_edge(&mut self, geom: LineOrPoint<T>, winding: WindingOrder) {
        ...
        match (le, re) {
            ...
            (None, Some(i)) => {
                ...
                self.push_link(i, left, winding_inverse(winding));
    ...
    fn push_link(&mut self, l: Link, pt: SweepPoint<T>, winding: WindingOrder) {
        if l.to_front {
            ...
            debug_assert_eq!(self.chains[l.idx].winding, winding_inverse(winding));
        } else {
    ...
    fn link_chains(&mut self, i: Link, j: Link, winding: WindingOrder) {
        if i.to_front {
            ...
            debug_assert_eq!(self.chains[i.idx].winding, winding_inverse(winding.clone()));
        } else {
        ...
        if j.to_front {
            ...
            debug_assert_eq!(self.chains[j.idx].winding, winding_inverse(winding));
        }
    }
}
